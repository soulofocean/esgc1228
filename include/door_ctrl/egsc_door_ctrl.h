#ifndef _EGSC_DOOR_CTRL_H_
#define _EGSC_DOOR_CTRL_H_

#include "../egsc_command.h"

// 门禁控制器 EGSC_TYPE_DOOR_CTRL -------------------------------------------------
typedef struct _egsc_door_ctrl_setting_parameters_param
{
	char alarm_timeout[32];    //必选，开门报警超时时间（s）
	char open_duration[32];    //必选，开门时长
}egsc_door_ctrl_setting_parameters_param;

typedef struct _egsc_door_ctrl_upload_record_param
{
	int pass_type;          //必选，进出类型，0为入；1为出；失败默认为0
	char *credence_no;      //可选，用来让平台区分开门次数
}egsc_door_ctrl_upload_record_param;

// 设备专有参数

// 服务器请求回调函数表
typedef struct _egsc_door_ctrl_cb_tbl
{
...
}egsc_door_ctrl_cb_tbl;

// 设备请求接口函数表
typedef struct _egsc_door_ctrl_if_tbl
{
