                ...
                {  
                    ...
                    if (len > 0){  
						...
						if(strcmp(buf,"open_door")==0){
							...
							//int a = mydev_upload_record(input_req_dev, input_req_cont);
							//printf("haha=%d\n",a);
						}
                    }
                    else   
...
{  
    ...
    if (argv[1])  
        ...
        myport = SOCKET_SERVER_PORT;  
          
    if (argv[2])  
        lisnum = atoi(argv[2]);  
    else  
        lisnum = SOCKET_SERVER_LISNUM;  
      
    if (argv[3]){ 
        printf("copy %d\n",sizeof(addr));
        strncpy(addr,argv[3],sizeof(addr));
    }
    return socketServerStart(myport, lisnum, addr); 
}  
